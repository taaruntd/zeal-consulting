...
export default function App(){
  ...
  return <>
    ...
    <main id="top">
      <section className="hero">
        ...
        <div className="heroCopy reveal">
          ...
          <div className="heroActions">
            <a className="button buttonGold" href="#contact">{content.hero.primary}<b>→</b></a>
            <a className="button buttonDark" href="#methodology">{content.hero.secondary}</a>
          </div>
        </div>
      ...
      <section className="industryBand" id="industries"><div className="shell">
        <SectionHeading eyebrow="INDUSTRIES" title="Built for scaling companies across complex sectors." center/>
      ...
      <section className="section systemsSection shell" id="work">
        <SectionHeading eyebrow="SELECTED SYSTEMS" title="We don't hand you a report. We leave behind infrastructure." desc="High-fidelity examples use anonymised labels and illustrative data. Detailed case studies are shared selectively after an introduction."/>
        <SystemsShowcase/>
      </section>

      <section className="proof section"><div className="shell proofGrid">
        <div><SectionHeading eyebrow="THE PATTERN" title="From fragmented execution to a scalable operating machine."/><p className="lead">{content.proof.narrative}</p></div>
        <div className="proofList">{content.proof.bullets.map(x=><div className="reveal" key={x}><span>✓</span>{x}</div>)}</div>
      </div></section>

      <section className="section shell">
        <SectionHeading eyebrow="WHAT WE BELIEVE" title="Practical systems. Clear ownership. Less operational drag." center/>
      ...
      <section className="section about" id="about"><div className="shell aboutGrid">
        <div className="aboutMark">Z</div>
        <div><div className="eyebrow">ABOUT ZEAL</div><h2>{content.about.title}</h2><p>{content.about.text}</p><blockquote>“{content.about.quote}”</blockquote></div>
      </div></section>

      <section className="closing" id="contact"><div className="closingMesh"/><div className="shell closingInner">
        <div className="eyebrow">READY TO BUILD YOUR OPERATING ADVANTAGE?</div>
        <h2>Your company has outgrown spreadsheets.<br/><span>Let’s build what comes next.</span></h2>
        <p>We intentionally limit active engagements so the work stays hands-on.</p>
        <div className="heroActions centerActions"><a className="button buttonGold" href={`mailto:${content.contact.email}`}>Let's Connect →</a><a className="button buttonDark" href={`tel:${content.contact.phone.replace(/\s/g,'')}`}>{content.contact.phone}</a></div>
      </div></section>
    </main>
